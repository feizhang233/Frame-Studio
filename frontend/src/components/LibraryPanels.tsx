...
function AssignmentList({
  ...
  return (
    <section className="library-apply-section">
      <div className="library-section-title"><div><span>ASSIGNMENT</span><strong>Apply to elements</strong></div><button type="button" onClick={onApplyAll}><Check size={15} /> Apply all</button></div>
        ...
        {model.elements.map((element) => {
          ...
          return (
            <button key={element.id} type="button" className={isAssigned ? 'is-assigned' : ''} onClick={() => onApply(element.id)}>
              ...
              <span>{isAssigned ? <><Check size={14} /> Assigned</> : 'Apply'}</span>
            </button>
          )
        })}
      </div>
...
export function MaterialLibraryPanel({ model, dispatch, elementDefaults, onElementDefaultsChange, onToggleCollapsed }: LibraryPanelProps) {
  ...
  return (
    <aside className="properties-panel library-panel">
      <LibraryHeader icon={<Library size={20} />} eyebrow="PROPERTY LIBRARY" title="Materials" subtitle={`${model.materials.length} defined`} onToggleCollapsed={onToggleCollapsed} />
      <div className="library-tabs" role="tablist" aria-label="Material definitions">
        ...
        <button className="library-add-tab" type="button" onClick={addMaterial} aria-label="Add material" title="Add material"><Plus size={18} /></button>
      </div>
      <div className="properties-scroll library-scroll">
        {!selected ? <div className="library-empty">Add a material to begin.</div> : <>
          <section className="library-editor-card">
            <div className="library-card-title"><div><span>MATERIAL DEFINITION</span><strong>{selected.name}</strong></div><button className="resource-delete-button" type="button" aria-label="Delete material" onClick={deleteSelected}><Trash2 size={15} /><span>Delete</span></button></div>
            ...
            <button className={`use-default-button ${elementDefaults.materialId === selected.id ? 'is-active' : ''}`} type="button" onClick={() => onElementDefaultsChange({ ...elementDefaults, materialId: selected.id })}><Check size={16} /> {elementDefaults.materialId === selected.id ? 'Default for new elements' : 'Use for new elements'}</button>
          </section>
...
export function SectionLibraryPanel({ model, dispatch, elementDefaults, onElementDefaultsChange, onToggleCollapsed }: LibraryPanelProps) {
  ...
  return (
    <aside className="properties-panel library-panel">
      <LibraryHeader icon={<Shapes size={20} />} eyebrow="PROPERTY LIBRARY" title="Sections" subtitle={`${model.sections.length} defined`} onToggleCollapsed={onToggleCollapsed} />
      <div className="library-tabs" role="tablist" aria-label="Section definitions">
        ...
        <button className="library-add-tab" type="button" onClick={addSection} aria-label="Add section" title="Add section"><Plus size={18} /></button>
      </div>
      <div className="properties-scroll library-scroll">
        {!selected ? <div className="library-empty">Add a section to begin.</div> : <>
          <section className="library-editor-card">
            <div className="library-card-title"><div><span>SECTION DEFINITION</span><strong>{selected.name}</strong></div><button className="resource-delete-button" type="button" aria-label="Delete section" onClick={deleteSelected}><Trash2 size={15} /><span>Delete</span></button></div>
            ...
            <button className={`use-default-button ${elementDefaults.sectionId === selected.id ? 'is-active' : ''}`} type="button" onClick={() => onElementDefaultsChange({ ...elementDefaults, sectionId: selected.id })}><Check size={16} /> {elementDefaults.sectionId === selected.id ? 'Default for new elements' : 'Use for new elements'}</button>
          </section>
...
export function ModelsPanel({
  ...
  return (
    <aside className="properties-panel library-panel">
      <LibraryHeader icon={<History size={20} />} eyebrow="MODEL BROWSER" title="Models" subtitle={`${history.length} saved`} onToggleCollapsed={onToggleCollapsed} />
      <div className="properties-scroll history-scroll">
        ...
        >
          <div className="model-section-header">
            <button className="model-section-toggle" type="button" onClick={() => setExamplesCollapsed((value) => !value)} aria-expanded={!examplesCollapsed}>
              {examplesCollapsed ? <ChevronRight size={18} /> : <ChevronDown size={18} />}
              <BookOpen size={19} />
            ...
            <button className="model-batch-delete" type="button" onClick={onDeleteAllExamples} disabled={examples.length === 0} aria-label="Delete all examples" title="Delete all examples"><Trash2 size={14} /><span>Delete all</span></button>
          </div>
          {!examplesCollapsed && (
            <div className="example-model-list">
              ...
              {examples.map((example) => (
                <article key={example.id} className="example-model-card">
                  <div className="example-card-heading"><div><strong>{example.name}</strong><span>{example.description}</span></div><button type="button" onClick={() => onDeleteExample(example.id)} aria-label={`Delete example ${example.name}`} title="Delete example"><Trash2 size={15} /></button></div>
        ...
        <section className="model-browser-section">
          <div className="model-section-header">
            ...
            <button className="model-batch-delete" type="button" onClick={onDeleteAllHistory} disabled={history.length === 0} aria-label="Delete all recent models" title="Delete all recent models"><Trash2 size={14} /><span>Delete all</span></button>
          </div>
          {!historyCollapsed && (
            <>
              ...
              <div className="history-list">
                {history.map((entry) => <article key={entry.id} className={`history-card ${draggedHistoryId === entry.id ? 'is-dragging' : ''}`} draggable onDragStart={(event) => startHistoryDrag(event, entry)} onDragEnd={() => { setDraggedHistoryId(null); setExampleDropActive(false) }}>
                  <div className="history-card-top"><GripVertical className="history-drag-handle" size={16} /><div><span>{entry.source === 'saved' ? 'SAVED' : 'ANALYZED'}</span><strong>{entry.name}</strong></div><div className="history-card-actions"><button type="button" onClick={() => onCreateExample(entry)} aria-label={`Add ${entry.name} to examples`} title="Add to Example models"><Plus size={15} /></button><button type="button" onClick={() => onDeleteHistory(entry.id)} aria-label={`Delete ${entry.name}`} title="Delete recent model"><Trash2 size={15} /></button></div></div>
                  ...
                  <button className="restore-button" type="button" onClick={() => onRestore(entry)}><RotateCcw size={15} /> Restore model</button>
                </article>)}
              </div>
            </>
          )}
...
export function ToolSetupPanel({
  ...
  const definitions = {
    node: { icon: <CircleDot size={20} />, eyebrow: 'CREATE TOOL', title: 'Node', description: 'Click on the grid to place a node. Coordinates snap to 0.25 m.' },
    element: { icon: <Waypoints size={20} />, eyebrow: 'CREATE TOOL', title: 'Element', description: 'Select an i-node and j-node to create a frame element.' },
    support: { icon: <Triangle size={20} />, eyebrow: 'CREATE TOOL', title: 'Support', description: 'Choose a support type, then click a node to assign it.' },
    load: { icon: <ArrowDownToLine size={20} />, eyebrow: 'CREATE TOOL', title: 'Load', description: 'Click a node for a nodal load or an element for a distributed load.' },
  } as const
  ...
  return (
    <aside className="properties-panel tool-setup-panel">
      ...
      <div className="properties-scroll">
        ...
        {tool === 'node' && <section className="tool-default-card"><span>GRID & SNAP</span><div className="tool-readout"><b>0.25</b><small>m increment</small></div><div className="tool-check"><Check size={15} /> Global X / Y coordinates</div></section>}
        ...
        {tool === 'load' && (
          <section className="tool-default-card">
            ...
            <div className="support-default-list load-default-list">
              <button type="button" className={nodalLoadDefaults.fy !== 0 && nodalLoadDefaults.fx === 0 && nodalLoadDefaults.mz === 0 ? 'is-active' : ''} onClick={() => onNodalLoadDefaultsChange({ fx: 0, fy: -1000, mz: 0 })}><ArrowDownToLine size={17} /><span><b>Force Y</b><small>Fy = −1.000 kN</small></span></button>
              <button type="button" className={nodalLoadDefaults.fx !== 0 && nodalLoadDefaults.fy === 0 && nodalLoadDefaults.mz === 0 ? 'is-active' : ''} onClick={() => onNodalLoadDefaultsChange({ fx: 1000, fy: 0, mz: 0 })}><CircleDot size={17} /><span><b>Force X</b><small>Fx = +1.000 kN</small></span></button>
              <button type="button" className={nodalLoadDefaults.mz !== 0 && nodalLoadDefaults.fx === 0 && nodalLoadDefaults.fy === 0 ? 'is-active' : ''} onClick={() => onNodalLoadDefaultsChange({ fx: 0, fy: 0, mz: 1000 })}><RotateCcw size={17} /><span><b>Moment</b><small>Mz = +1.000 kN·m</small></span></button>
            </div>
